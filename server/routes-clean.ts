...
const createCrudRoutes = (
  ...
  app.get(`/api/${basePath}`, async (req: Request, res: Response) => {
    try {
      ...
      const items = await (storage as any)[`get${storageMethodPrefix}s`]();
      res.json(items);
    } catch (error) {
  ...
  app.get(`/api/${basePath}/:id`, async (req: Request, res: Response) => {
    try {
      const storage = await getStorage();
      const item = await (storage as any)[`get${storageMethodPrefix}`](req.params.id);
      if (!item) {
  ...
  app.post(`/api/${basePath}`, async (req: Request, res: Response) => {
    try {
      ...
      const item = await (storage as any)[`create${storageMethodPrefix}`](data);
  ...
  const updateHandler = async (req: Request, res: Response) => {
    try {
      const storage = await getStorage();
      const item = await (storage as any)[`update${storageMethodPrefix}`](req.params.id, req.body);
      if (!item) {
        return handleNotFound(res, entityName);
      }
      res.json(item);
  ...
  app.delete(`/api/${basePath}/:id`, async (req: Request, res: Response) => {
    try {
      const storage = await getStorage();
      const success = await (storage as any)[`delete${storageMethodPrefix}`](req.params.id);
      if (!success) {
        return handleNotFound(res, entityName);
      }
...
const routeConfigs = [
  ...
  { path: 'payroll', entity: 'Payroll', method: 'Payroll' },
