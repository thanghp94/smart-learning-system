...
const FilterButton: React.FC<FilterButtonProps> = ({
  ...
  return <div className="flex flex-wrap gap-1 items-center">
      {!isMobile && <Button variant="outline" size="sm" className="h-8 mr-1" disabled>
          <Filter className="h-4 w-4 mr-2" />
          {label}
        </Button>}
      
      <div className="flex flex-wrap gap-1 items-center">
        {categories.map(category => <Popover key={category.name}>
        ...
        {getActiveFilterCount() > 0 && <Button variant="ghost" size="sm" className="h-8 px-2" onClick={clearFilters}>
